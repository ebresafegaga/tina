...
type toplevel =
  | Claim of Loc.t * VarName.t * T.t
  | Def of Loc.t * VarName.t * t
  | VariantDef of Loc.t * DataName.t * (VarName.t * T.t list) list
  | RecordDef of Loc.t * DataName.t * (FieldName.t * T.t) list
  | AbilityDef of Loc.t * VarName.t * T.t list
  | Expression of t
