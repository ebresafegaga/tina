...
type toplevel = Def of string * expression | Expression of expression
...
and gstate expr =
  match expr with
  | A.LitBool _ | A.LitFloat _ | A.LitString _ | A.LitInteger _ | A.Variable _
  | A.Application _ | Record _ | RecordIndex _ | A.Absurd _ | A.Fn _ ->
      [ Return (Some (gexpr expr)) ]
  | A.Let (x, expr, body) -> Let (x, gexpr expr) :: gstate body
  | A.If (p, pt, pf) ->
      let p = variable p in
...
let rec gen_expression = function
  ...
  | Variable s -> s
  | Application (f, args) ->
      Printf.sprintf "(%s) (%s)" (gen_expression f) (pp_list args gen_expression)
  | Fn (args, body) ->
      Printf.sprintf {| (%s) => { %s } |}
        (pp_list args VarName.to_string)
        (combine_statement (List.map gen_statement body))
  | Record fields ->
      Printf.sprintf "{ %s }"
        (pp_list fields (fun (name, expr) ->
...
and gen_statement = function
  ...
  | If (p, pt, pf) ->
      Printf.sprintf
        ...
        (gen_expression p)
        (combine_statement (List.map gen_statement pt))
        (combine_statement (List.map gen_statement pf))
  | Let (x, e) ->
      Printf.sprintf "let %s = %s" (VarName.to_string x) (gen_expression e)
...
let gen_toplevel = function
  ...
  | Expression e ->
      Printf.sprintf "%s; %s;" (gen_expression e)
        (combine_statement (List.map gen_statement @@ remove_level ()))
